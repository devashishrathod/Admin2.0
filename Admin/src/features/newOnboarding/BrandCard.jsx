...
export default function BrandCard({ verification, onOpen }) {
  ...
  return (
    <button
      ...
      className="group relative flex flex-col overflow-hidden rounded-3xl border border-neutral-800 bg-neutral-900 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:border-neutral-700 hover:shadow-lg hover:shadow-black/30"
    >
      {/* Ambient accent glow */}
      <div
      ...
      <div className="relative flex flex-col p-4">
          ...
          {KEY_FLAGS.map((f) => (
            <span
              ...
              className={`flex items-center gap-1 text-[11px] font-medium ${
                flags[f.key] ? "text-emerald-400" : "text-red-400/80"
              }`}
            >
        ...
        <div className="mb-3.5 flex items-center gap-2 rounded-xl border border-neutral-800/80 bg-neutral-950/60 px-2.5 py-1.5">
          <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-neutral-800 text-neutral-400">
            <User size={12} />
          </span>
          <div className="min-w-0 leading-tight">
            <p className="truncate text-[12px] font-semibold text-neutral-200">
        ...
        {verification.remarks?.length > 0 && (
          <div className="mb-3.5 flex items-start gap-1.5 rounded-xl border border-amber-400/20 bg-amber-400/5 px-2.5 py-2 text-[11px] text-amber-300">
            <AlertTriangle size={12} className="mt-0.5 shrink-0" />
            <span className="line-clamp-2">{verification.remarks[0]}</span>
          </div>
        )}

        <div className="flex items-center justify-between rounded-xl bg-neutral-950/60 px-3 py-2.5 text-[11px] text-neutral-500">
          <span className="flex items-center gap-1">
            <RefreshCcw size={11} />
            {verification.submissionCount} submission{verification.submissionCount === 1 ? "" : "s"}
          </span>
          <span className="font-semibold text-emerald-400 transition-transform group-hover:translate-x-0.5">
            View →
          </span>
        </div>
      </div>
